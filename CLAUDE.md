...
- `data/daily-<region>.json` is the site's store: every day's summary since Oct 2024, one file per region. The build reads it first and only fetches days that aren't final yet (today, plus any day stored with a short slot count). Commit `data/` with the source. Delete a file to force a full refetch for that region. Logic in `src/lib/store.mjs`.
