...
- `dist/` is the output. Never edit it by hand; rebuild instead. One static page per region × time range (14 × 6 = 84). London (region C) is at the root: `dist/index.html` (1 year), `dist/90-days/`, `dist/all/`, and one folder per calendar year (`dist/2024/` onwards, derived from the product start year to the current year). Every other region sits under its name as a slug: `dist/yorkshire/`, `dist/yorkshire/90-days/`, `dist/south-eastern-england/2025/`. The single-letter paths (`dist/m/…`) and `dist/london/…` are meta-refresh redirect stubs (`src/redirect.html`) to the canonical page. `DEFAULT_REGION` in `src/lib/regions.mjs` picks the root region. Ranges are in `src/lib/ranges.mjs`, regions in `src/lib/regions.mjs`. Both switchers are plain links (with a `#daily-prices` anchor for the no-JS path); `src/switcher.js` enhances them to swap content in place, and `style.css` names the pill and labels for the view transition.
