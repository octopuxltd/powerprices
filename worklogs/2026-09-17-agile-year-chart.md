...
## Gotchas
- **Smooth over the whole store, not the page's days.** The monthly trend was first smoothed over each page's own days, so at a page edge the 30-day window was cut in half and 31 Dec 2024 didn't meet 1 Jan 2025 (Paul spotted the line leaving one page under 20p and entering the next above it). Both trend lines now come from one `smoothedAverages(allDays)` map keyed by date.
- **GitHub Pages sends `cache-control: max-age=600`.** For 10 minutes after a deploy a browser can keep serving old pages, and the in-place switcher fetched through that cache, so region links appeared to still use the old letter URLs. `switcher.js` now fetches with `cache: 'no-cache'` (revalidates via ETag). A hard refresh clears the initial page itself.
